...
"""
ref: https://github.com/ML4ITS/TimeVQVAE-AnomalyDetection/blob/main/utils/__init__.py
"""
...
def time_to_timefreq_wavelet(x, scales: np.ndarray, C: int, wavelet: str = "morl"):
    """
    Perform Wavelet Transform and separate into low, mid, and high scales while preserving channel dimensions.
    
    Args:
        x: Input tensor of shape (B, C, L).
        low_scale: Integer scale for low-frequency range.
        mid_scale: Integer scale for mid-frequency range.
        high_scale: Integer scale for high-frequency range.
        C: Number of input channels.
        wavelet: Wavelet function name (default: 'morl').

    Returns:
        Tensor of shape (B, C, 3, T), where 3 represents low, mid, and high scales.
    """

    B, C, L = x.shape  # Batch, Channel, Signal Length

    # Combine batch and channel dimensions for processing
    x = rearrange(x, "b c l -> (b c) l")  # Shape: (B*C, L)
    device = x.device
    # Define scales
    coeffs, _ = pywt.cwt(x.cpu().numpy(), scales, wavelet)  # Coefficients shape: (3, B*C, L)
    coeffs = torch.tensor(coeffs, dtype=torch.float32, device=device)
    coeffs = rearrange(coeffs, "h (b c) t -> b c h t", b=B, c=C)  # Shape: (B, C, 3, L)

    return coeffs  # Output shape: (B, C, 3, L)



def timefreq_to_time_wavelet(coeffs, scales: np.ndarray, C:int, wavelet: str = "morl"):
    """
    Approximate reconstruction of the original signal from wavelet coefficients.
    
    Args:
        coeffs: Input tensor of wavelet coefficients of shape (B, C, 3, L),
                where 3 corresponds to low, mid, and high frequency scales.
        scales: Scales array [low_scale, mid_scale, high_scale].
        wavelet: Wavelet function name (default: 'morl').

    Returns:
        Reconstructed tensor of shape (B, C, 1, L), approximating the original signal.
    """
    # Obtain the normalization constant for the wavelet function
    mwf = pywt.ContinuousWavelet(wavelet).wavefun()  # Get wavelet function details
    y_0 = mwf[0][np.argmin(np.abs(mwf[1]))]  # Value at time 0 (center of wavelet)

    B, C, H, L = coeffs.shape  # Batch, Channels, 3 Scales, Signal Length

    # Combine batch and channel dimensions for processing
    coeffs = rearrange(coeffs, "b c h l -> (b c) h l")  # Shape: (B*C, 3, L)


    # Reconstruct signals
    reconstructed_signals = []
    for batch_channel in range(coeffs.shape[0]):
        # Extract coefficients for the current (batch, channel)
        current_coeffs = coeffs[batch_channel].detach().cpu().numpy()  # Shape: (3, L)

        # Scale each coefficient by its corresponding scale factor
        r_sum = np.transpose(np.sum(np.transpose(current_coeffs) / scales**0.5, axis=-1))  # Shape: (L,)
        reconstructed_signal = r_sum * (1 / y_0)  # Normalize with wavelet center value
        reconstructed_signals.append(reconstructed_signal)

    # Convert reconstructed signals back to tensor
    reconstructed_signals = torch.tensor(reconstructed_signals, dtype=torch.float32, device = coeffs.device)  # Shape: (B*C, L)
    reconstructed_signals = scale(rearrange(reconstructed_signals, "(b c) l -> b c l", c=C))  # Shape: (B, C, L)

    return reconstructed_signals


def time_to_timefreq_3C(x, n_fft: int, C: int):
    x = rearrange(x, 'b c l -> (b c) l')

    stft_complex = torch.stft(
        x, n_fft, normalized=True, return_complex=True,
        window=torch.hann_window(window_length=n_fft, device=x.device)
    )

    magnitude = torch.abs(stft_complex)  # (B, H, W)
    # Real 및 Imaginary 부분 분리
    real = torch.real(stft_complex)  # (B, H, W)
    imag = torch.imag(stft_complex)  # (B, H, W)
    
    # Magnitude Temporal Gradient
    temporal_gradient = torch.diff(magnitude, dim=-1, prepend=magnitude[..., :1])  # (B, H, W)


    x_transformed = torch.stack([real, imag, temporal_gradient], dim=1)  # (B, 3, H, W)
    return x_transformed  # (B, 3, H, W)
...
def set_window_size(data):
    ...
    end_index = length
    step_size = 1000
    current_start_index = start_index  # Start index for the current instance
    
    while current_start_index < end_index:
        slice_end = min(current_start_index + step_size, end_index)  # Ensure slice doesn't exceed 5500
        ppg_slice = data[current_start_index:slice_end]
        
        # Filter the signal
        filtered_ppg = filter(ppg_slice, lowcut=0.5, highcut=8, fs=fs)  # PPG typically uses a lower frequency range
        try:
            # Process PPG peaks
            ppg_peaks, _ = hp.process(filtered_ppg, sample_rate=fs, bpmmin=40, bpmmax=180)
            ppg_peaks = ppg_peaks['peaklist']
            
            if len(ppg_peaks) > 1:  # Check if there are enough peaks
                peak_intervals = np.diff(ppg_peaks) / fs  # Calculate peak intervals
                
                # Append intervals to hr_1208_data
                hr_list.append(peak_intervals)
        except hp.exceptions.BadSignalWarning as e:
            continue
                 
        current_start_index += step_size  # Move to the next slice
        hr_list_flatten = np.hstack(hr_list)
        hr_data.append(np.hstack(hr_list_flatten))
        hr_list = []

    if not hr_data:
        raise ValueError("Error: No valid HR data found! Ensure input data is valid.")
    
    window_size = int(np.median(np.hstack(hr_data)) * fs)
  
    return window_size

'''

# Bandpass filter to remove noise from the signal
...
def set_window_size(data, step_size: int = 100):
    fs = 100  
    window_length = 1000       
    length = len(data)
    hr_data = []  
    ...
    for start in range(0, length - window_length + 1, step_size):
        window = data[start:start + window_length]
        

        filtered_ppg = filter(window, lowcut=0.5, highcut=8, fs=fs)
        try:
            ...
            if len(ppg_peaks) > 1: 
                ...
                hr_data.append(peak_intervals)
        except hp.exceptions.BadSignalWarning:
           
            continue

    if not hr_data:
        raise ValueError("Error: No valid HR data calculated!")

    all_intervals = np.hstack(hr_data)
    window_size = int(np.median(all_intervals) * fs)
...
def pre_window_size(num_idx,n_periods: int):
    period_data = get_root_dir().joinpath('preprocessing', 'pre_train_period.csv')
    df = pd.read_csv(str(period_data))
    single_period = int(df['period'][num_idx].item())
...
def set_window_size(dataset_id, n_periods: int):
    period_data = get_root_dir().joinpath('preprocessing', 'test_dataset_periods.csv')
    df = pd.read_csv(str(period_data))
    single_period = int(df.loc[df['dataset_idx'] == int(dataset_id)]['period'].item())
    #single_period = int(df.query(f"dataset_idx == {dataset_id}")['period'])
    return single_period * n_periods

'''
def resampling(x, out_length: int):
    if x.ndim == 1:
        x = x.reshape(1, -1)
    out = resample(x, out_length, axis=1)
    return out.flatten()  
