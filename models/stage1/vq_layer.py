"""
reference: https://github.com/ML4ITS/TimeVQVAE-AnomalyDetection/blob/main/models/stage1/vq.py
...
#     - codebook: (K, dim)
#     """
#     norm_cb = (codebook - codebook.mean(dim=0, keepdim=True))  # (K d)
#     norm_cb = F.normalize(norm_cb, p=2, dim=0)  # (K d)
#     corr_mat = torch.mm(norm_cb.T, norm_cb)  # (d d)
...
#     - codebook: (K dim)
#     """
#     std_cb = torch.sqrt(codebook.var(dim=0) + 1e-4)
...
class EuclideanCodebook(nn.Module):
    ...
    def forward(self, x, svq_temp=None):
        ...
        embed = self.embed.detach()  #self.embed if not self.learnable_codebook else self.embed.detach()
        embed = embed.t()
        ...
        if self.training:
            ...
            temp = 0 if isinstance(svq_temp, type(None)) else svq_temp
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        # learnable_codebook = (orthogonal_reg_weight > 0) or (codebook_weight_decay > 0)
    ...
    def forward(self, x, svq_temp=None):
        ...
        quantize, embed_ind = self._codebook(x, svq_temp=svq_temp)
        ...
        #     cov_loss = cov_loss_fn(self.codebook)
        #     var_loss = var_loss_fn(self.codebook)
        #     vq_loss['cov_loss'] = cov_loss
        #     vq_loss['var_loss'] = var_loss
        #     vq_loss['loss'] += cov_loss * self.cov_loss_weight + var_loss * self.var_loss_weight
...
if __name__ == '__main__':
    ...
    vq = VectorQuantize(dim=D, codebook_dim=D*2, codebook_size=512, codebook_weight_decay=0.001)

    quantize, vq_ind, vq_loss, perplexity = vq(x)
    print(vq_ind[0])

    for name, params in vq.named_parameters():
        print('name:', name)
        if not name.startswith('_codebook'):
            print('yo')
