...
from models.stage2.layer_norm import layer_norm_linear_fn, RMSNorm
...
class BiMambaLayerV2(nn.Module):
    ...
    def __init__(self, dim, d_state=16, d_conv=3, dt_rank="auto", drop_path=0.1):
        ...
        #self.conv_2d = nn.Conv2d(dim, dim // 2, kernel_size=(1, d_conv), bias=True, padding="same", groups=dim // 2)


        A = repeat(torch.arange(1, self.d_state + 1, dtype=torch.float32), "n -> d n", d=dim // 2).contiguous()
        #Ab = repeat(torch.arange(1, self.d_state + 1, dtype=torch.float32), "n -> d n", d=dim // 2).contiguous()
        self.A_log = nn.Parameter(torch.log(A))
        #self.A_log.no_weight_decay= True
        
        self.Ab_log = nn.Parameter(torch.log(A.clone()))
        #self.Ab_log.no_weight_decay= True
        
        self.D = nn.Parameter(torch.ones(dim // 2))
        #self.D.no_weight_decay = True
    ...
    def forward(self, x):
        ...
        #x = layer_norm_linear_fn(x, self.norm_weight, self.norm_bias, self.linear_weight, self.linear_bias)
        #x_proj = rearrange(self.in_proj(x), "b l c -> b c l")
        ...
        ## conv 2d
        #x_proj = rearrange(x_proj, "(b h) c w -> b c h w", h=3)
        #x_proj = self.conv_2d(x_proj)
        #x_proj = rearrange(x_proj, "b c h w -> (b h) c w")       
...
class TransitionBlock(nn.Module):
    def __init__(self, dim, expansion: int):
        ...
        #self.depthwise_conv = nn.Conv1d(expansion * dim, expansion * dim, kernel_size=3, padding="same", groups=expansion * dim)
...
class BiMambaBlockV2(nn.Module):
    def __init__(self, dim, n_layer=1, drop_path=0.1):
        super().__init__()
        
        self.mamba_layers = nn.ModuleList([
            BiMambaLayerV2(dim, d_state=16, d_conv=3, drop_path=drop_path) for _ in range(n_layer)
        ])
        #self.conv = nn.Conv2d(dim, dim, kernel_size=3, padding="same", groups=dim)
